...
remaining_musculus_contigs = ['GL456210.1',
 'GL456211.1',
 'GL456212.1',
 'GL456216.1',
 'GL456219.1',
...
with open("MUSCULUS_genome_contigs.txt", "r") as f:
    file = f.readlines()
    for line in file:
        contig = line.split("\t")
        musculus_list.append((contig[0], contig[4]))
        musculus_dict[contig[0]] = contig[4]

values = musculus_dict.values()

for i in a:
    if i in musculus_chromosomes:
        final_dict[i] =  musculus_dict[i]
        musculus_chromosomes.remove(i)
    if i in remaining_musculus_contigs:
        final_dict[i] = i

fai_f = open("MUSCULUS_genome.fasta.fai", "r")
fai_file = fai_f.readlines()

fai_contigs = []
for j in fai_file:
    fai_contigs.append(j.split("\t")[0])

for key, value in final_dict.items():
    if value not in fai_contigs:
        print("Absent Value: %s " % value)
    else:
        print("Present Value: %s " % value)
