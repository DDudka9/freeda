...
def clone_cds(preselected_exons_overhangs, most_intronic_contigs, protein_name,
              genome_name, final_exon_number, original_exons, msa_path, microexons):
    ...
    for e in range(1, len(preselected_exons_overhangs) + 1):
        ...
        if not preselected_exons_overhangs[e]:
        ...
        # if any contig has been already analyzed for hd -> skip all
            ...
            for winner in winners_hd_normalized:
                ...
                for exon_nr in exons:
                    total_hd_normalized += hamming_distance_to_original_species(original_exons,
                                                                                exon_nr, winner,
                ...
                for seq in preselected_exons_overhangs[exon]:
                    ...
                    if contig[0] == seq[0] and cds_composition[exon] == "":
                        in_filename, out_filename = generate_single_exon_msa(seq[1], contig[0], exon,
                                                                             protein_name, original_exons, msa_path,
                                                                             seq[2])
                        ...
                        original_exon, locus_exon = index_positions_exons(aligned_seqs)
                        # check frameshift (DISABLED)
                        frameshift = False
                        # frameshift = check_frameshift(original_exon, locus_exon)

                        # check if its not the last exon
                        if exon != list(original_exons.keys())[-1] and frameshift is False:
                        ...
                        if frameshift is True and exon != list(original_exons.keys())[-1]:
                            # log the frameshift
                        ...
                        # if last exon
                        if exon == list(original_exons.keys())[-1]:
...
def hamming_distance_to_original_species(original_exons, exon_nr, winner,
    ...
    with open(filename, "w") as f:
        ...
        f.write(original_exons[exon_nr][0] + "\n")
        f.write(original_exons[exon_nr][1] + "\n")
    ...
    hd = hamming_distance_frameshift(original_exon, compared_exon, exon_nr, original_exons)
...
def check_frameshift(original_exon, compared_exon):
    ...
    if "-" in oe.values() and end_of_contig is False:
        # count them
        count = 0
        lam = lambda pos: count + 1
...
def check_stop_codon(last_original_exon, last_locus_exon, exon_number):  # works well
    # define the STOP codon based on the Mm last exon
    tag_codon_pos = last_original_exon.rfind("TAG")
    tga_codon_pos = last_original_exon.rfind("TGA")
    taa_codon_pos = last_original_exon.rfind("TAA")
