...
def check_all_structures(wdir, original_species):
    ...
    if missing_structures_final:
        print(
            "...WARNING... (FREEDA) I did not find clear structure prediction models for: %s" % missing_structures_final)
...
def run_pymol(wdir, original_species, result_path, protein, offset):
    ...
    if not get_pymol_script(wdir, original_species, dictionary, protein, protein_path, offset):
        return False
...
def get_pymol_script(wdir, original_species, dictionary, protein, protein_path, offset):
