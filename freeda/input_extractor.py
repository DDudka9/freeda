...
def fetch_structure_prediction(wdir, original_species, protein, possible_uniprot_ids):
    ...
    # didnt find structure
        ...
        print(
            "...SUGGESTION...: You can use your own model (ex. from PDB; fragments are ok but no sequence mismatches!)\n")
...
def generate_reference_genome_object(wdir, original_species, reference_genome_name):
    ...
    if original_species in human_names:
        ...
        release = 100
        reference_genome_contigs_dict = reference_genome_dict_generator.get_reference_genome_contigs_dict(
            original_species)
    ...
    reference_genome_present = check_genome_present(reference_genomes_path, reference_genome_name,
...
def extract_input(wdir, original_species, reference_genome_name, reference_genomes_path,
    ...
    transcript, selected_transcript_id, gene_id, contig, strand, UTR_5, UTR_3, cds_sequence_expected, matching_length = extract_cds(
        ensembl,
        original_species, coding_sequence_input_path, protein, biotype, model_seq)
    # find protein sequence
    model_matches_input = extract_protein(original_species, blast_input_path, protein, transcript, model_seq,
                                          matching_length)
    # find gene sequence
    ...
    # find exons sequence
    input_correct, microexon_present = extract_exons(wdir, original_species, protein, exons_input_path,
                                                     reference_genomes_path, reference_genome_name,
                                                     contig, strand, transcript, reference_genome_contigs_dict, UTR_5,
                                                     UTR_3, cds_sequence_expected)

    return input_correct, model_matches_input, microexon_present


def extract_protein(original_species, blast_input_path, protein, transcript, model_seq, matching_length):
...
def extract_exons(wdir, original_species, protein, exons_input_path, reference_genomes_path, reference_genome_name,
    ...
    for exon in exon_intervals:
        ...
        exon_fasta_sequence = get_single_exon(original_species, protein, reference_genome_contigs_dict,
                                              reference_genomes_path, reference_genome_name, rules, contig, strand,
                                              number, start, stop)
...
def extract_cds(ensembl, original_species, coding_sequence_input_path, protein, biotype, model_seq):
    ...
    gene_id, contig, strand, transcript_name, start, end, cds_sequence_expected, length = all_transcripts_dict[
