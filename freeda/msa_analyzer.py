...
def analyse_msa(wdir, msa_path, protein_name, genome_name, original_exons, microexons, expected_exons):
    final_exon_number = len(original_exons)
    ...
    for path in paths:
        ...
        exons, possible_retrotransposition, synteny, RETRO_score, duplication_score \
            = exon_finder.find_exons(cds, locus, gene, contig_name, original_exons, expected_exons, microexons)
        ...
        if (possible_retrotransposition is True and RETRO_score >= 0.4) or duplication_score < 0.5:
            continue
    ...
    preselected_exons_overhangs = preselect_exons_overhangs(cloned_exons_overhangs, expected_exons)
    ...
    cloned_cds = cds_cloner.clone_cds(preselected_exons_overhangs, most_intronic_contigs,
                                      protein_name, genome_name, final_exon_number,
                                      original_exons, msa_path, microexons)
...
def preselect_exons_overhangs(cloned_exons_overhangs, expected_exons):
    intronic_exons = []
