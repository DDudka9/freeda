...
def find_exons(cds, locus, gene, contig_name, original_exons, expected_exons, microexons):  # works well
    ...
    for position in cds:
        ...
        if exon_checked is False and cds[position] == gene[position]:
            ...
            if locus[position] in "ACTG":
                ...
                elif exon_number != 1 and homology_check(position, last_bp, cds, locus, gene) is True:
        ...
        if exon_checked is True and cds[position] == "-" and gene[position] != "-":
            ...
            if exon_number == list(original_exons.keys())[-1]:

                # check synteny
                # WARNING: average 800bp 3'UTRs in mammals make synteny check at C-term not very efficient
            ...
            # check for very divergent introns (will be counted intronic) -> does not allow divergent introns in last exon
                    ...
                    and exon_number != list(original_exons.keys())[-1] \
        ...
        if last_bp is True:
            ...
            if N_term_retrotransposition is True or C_term_retrotransposition is True:
                ...
                # RETRO at N-term
                    ...
                    possible_retrotransposition is True
                    nr_of_RETRO_exons += 1
                    if introny_at_cterm is True:
                        introny = True

                        # check insertions
                        big_insertion = check_insertion(contig_name, insertion, exon_number, original_exons,
                                                        insertion_with_N)
            ...
            if divergent_introns is True and introny_at_nterm is False and introny_at_cterm is False \
                ...
                big_insertion = check_insertion(contig_name, insertion, exon_number, original_exons, insertion_with_N)
                # message = "insertion: %s" % str(insertion)
                # print(message)
                # logging.info(message)
            ...
            if introny_at_nterm is True or introny_at_cterm is True:
                ...
                # CATCH NO CALL DECISIONS
                    ...
                    message = "No call in introny: \nintrony = %s, \nN_term introny = %s, \nC_term introny = %s, \ndivergent exons = %s" \
                              ...
                              % (
                                introny, introny_at_nterm, introny_at_cterm, divergent_introns, N_term_retrotransposition,
                                C_term_retrotransposition)
                    print(message)
                    logging.info(message)
            ...
            # CATCH NO CALL DECISIONS
                ...
                message = "No call at all: \nintrony = %s, \nN_term introny = %s, \nC_term introny = %s, \ndivergent introns = %s" \
                          "\nsingle exon = %s, \nN_term_retrotransposition = %s, \nC_term_retrotransposition = %s" \
                          % (introny, introny_at_nterm, introny_at_cterm, divergent_introns, single_exon,
                             N_term_retrotransposition, C_term_retrotransposition)
                print(message)
                logging.info(message)
    ...
    if exon_number != len(original_exons):
...
def check_introny(position, last_bp, cds, locus, gene):  # works well
    ...
    if last_bp is False:
        ...
        elif extension1 - 1 > position >= extension2 - 1:
            end = position - extension2
            extension = extension2

        elif extension2 - 1 > position >= extension3 - 1:
    ...
    if last_bp is True:
        ...
        elif position + extension1 > max_length + 1 >= position + extension2:
            end = position + extension2 - 1
            extension = extension2

        elif position + extension2 > max_length + 1 >= position + extension3:
...
def homology_check(starting_position, last_bp, cds, locus,
                   gene):  # runs only when ambigous introny (100bp stretch with 0.80 homology) or RETRO suspected
...
def check_insertion(contig_name, insertion, exon_number, original_exons, insertion_with_n):
    big_insertion = False
    ...
    if exon_number == list(original_exons.keys())[-1] and (insertion >= 180 or (insertion >= 10 and insertion % 3 != 0)):
